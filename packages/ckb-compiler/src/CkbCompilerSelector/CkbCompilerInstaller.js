...
export default class CkbCompilerInstaller extends PureComponent {
  constructor (props) {
    super(props)
  ...
  fetchRemoteVersions = async () => {
    ...
    let versions
    try {
      versions = await ckbCompiler.invoke('remoteVersions', 10)
    } catch (e) {
  ...
  render () {
    const title = (
      <div key='icon-downloading-ckb'>
        <i className='fas fa-spinner fa-spin mr-2' />Downloading CKB Compiler {this.state.downloadVersion}...
      </div>
    )
    return (
      <React.Fragment>
        <UncontrolledButtonDropdown size={this.props.size}>
          ...
          <DropdownMenu right={this.props.right}>
            <DropdownItem header className='small'>Available CKB Compilers</DropdownItem>
            {this.renderVersions()}
          </DropdownMenu>
        </UncontrolledButtonDropdown>
        <Modal
        ...
        >
          <div className='rounded overflow-hidden'>
            <Terminal
              active
              logId='download-ckb-compiler'
              height='300px'
              cmd={`docker pull nervos/ckb-riscv-gnu-toolchain:${this.state.downloadVersion}`}
